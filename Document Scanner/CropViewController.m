...
- (void)viewDidLoad {
    ...
    _detectedImage = [[UIImageView alloc] initWithFrame:CGRectMake(margin, [CropperConstantValues pictureSelectorHeaderViewHeight] + margin, self.view.frame.size.width - margin * 2, self.view.frame.size.height -[CropperConstantValues pictureSelectorFooterViewHeight] -[CropperConstantValues pictureSelectorHeaderViewHeight] - margin * 2)];
...
-(void)autoRectangleDetect{
    ...
    if (_magnetEnabled) {
        ...
        if (_detectedRectangleFeature)
        {
            ...
            UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"Внимания" message:@"Мы не можем обнаружить документ, пожалуйста переснимите документ." delegate:nil cancelButtonTitle:nil otherButtonTitles:@"Ok", nil];
            [alertView show];
